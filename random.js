$(document).ready(function(){
	...
	/* window.randListInstruction = "A tool that randomizes your list of names/items &nbsp; <span class='glyphicon glyphicon-user'></span>";
	window.randIntegerInstruction = "A tool to generate a single random integer within certain range &nbsp; 	<span class='glyphicon glyphicon-cog'></span>";
	window.randIntegListInstruction = "Randomizes and generates a random list of integers according to range you specified &nbsp; <span class='glyphicon glyphicon-download-alt'></span>";
    */
	
	var howTo = "<br><br>1.Вставте список в поле ввода или выберите диапазон цифр.<br>2.Нажмите кпопку 'Старт'.<br>3.С помощью 'Copy'  можно скопировать результат.";
	
	window.randListInstruction = "Генерирует случайный порядок Вашего списка. Каждый пункт списка или имена вводить с новой строки, через Enter  &nbsp; <span class='glyphicon glyphicon-user'></span>" + howTo;
	window.randIntegerInstruction = "Генерирует одно случайно число в заданном Вами диапазоне &nbsp; 	<span class='glyphicon glyphicon-cog'></span>" + howTo;
	window.randIntegListInstruction = "Генерирует случайный список номеров в заданном Вами диапазоне &nbsp; <span class='glyphicon glyphicon-download-alt'></span>" + howTo;
	...
	function randListCoreAction () 
	{
		...
		//alert (inputArray.length); //pass for checking in isRandom()
		window.lengh = inputArray.length;
...
function(){ //hover off
          ...
          $("#textChange").stop().fadeOut(900,function(){  $(this).html("Генератор номеров"/* "&nbsp;Randomize service" */) }).fadeIn(900);
